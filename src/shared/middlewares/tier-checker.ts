...
// import { TierLimit, Wallet } from "@/shared/database/models";
// import { getUserFromRequest } from "@/shared/middlewares/auth/auth-helpers";
...
export const checkTransactionLimit = () => {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        const wallet = await Wallet.findOne({ where: { user_id: user.userId } });
        if (!wallet) {
            return res.status(500).json({ error: "Failed to fetch wallet" });
        }
        ...
        // Weekly limit
        if (wallet.weeklySpent + amount > tierLimit.weeklyLimit) {
            return res.status(403).json({
                error: "Weekly transaction limit exceeded",
                weekly_spent: wallet.weeklySpent,
                weekly_limit: tierLimit.weeklyLimit,
                remaining: tierLimit.weeklyLimit - wallet.weeklySpent,
            });
        }

        // Monthly limit
        if (wallet.monthlySpent + amount > tierLimit.monthlyLimit) {
            return res.status(403).json({
                error: "Monthly transaction limit exceeded",
                monthly_spent: wallet.monthlySpent,
                monthly_limit: tierLimit.monthlyLimit,
                remaining: tierLimit.monthlyLimit - wallet.monthlySpent,
            });
        }
